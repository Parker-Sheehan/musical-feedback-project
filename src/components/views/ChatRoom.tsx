...
const ChatRoom: FC<ChatRoomProps> = ({
  ...
  const chatRoom = useMemo(() => {
    ...
    let updatedRoom = chatRooms.filter(
      (chatRoom) => chatRoom.chatRoomId === currentChatRoom
    )[0];
    ...
    {showChatMobile ? (
      ...
      <div className="h-full w-full p-1">
        <div className="h-full text-wrap rounded">
          ...
          {chatRoom &&
            (chatRoom.user1Id === loggedInUser.userId ? (
              <Link
          ...
          <div
            className="  h-4/5 w-full flex flex-col overflow-scroll max-w-[240px]"
            ref={messagesRef}
          >
            {messagesArray &&
              messagesArray.map((mappingMessage, index) => {
                if (
                  !messagesArray[index - 1] ||
                  mappingMessage.createdAt.split("T")[0] !==
                    messagesArray[index - 1].createdAt.split("T")[0]
                ) {
                  // console
