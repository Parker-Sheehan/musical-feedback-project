...
const ReviewSection: FC<ReviewSectionProps> = ({reviewForId, songId, artistQuestion}) => {
  ...
  const submitReviewHandler = async () => {
    ...
    if (
      ...
      let response = await instance.post(`http://localhost:3000/postCritique/${loginState.userId}`, bodyObj);
  ...
  return (
    <div className="flex flex-col w-11/12 bg-background2 items-center rounded-lg mb-3 ">
      ...
      <p className="text-background">Aesthetic question</p>
        <p className="text-background w-11/12">Describe the aesthetic of this song, what sort of imagery or feelings does it invoke?</p>
        <textarea ref={aestheticCritiqueRef} className="w-11/12 h-50"></textarea>
      </div>
      <div className="bg-red-500 size-4/5 flex flex-col items-center m-6 rounded-md h-72 justify-evenly">
      <p className="text-background">Technical question</p>
        <p className="text-background w-11/12">What technical aspects of the production that you think could be improved{"(EQ, ryhthm, mix, sound design, musicality, etc...)"}? which ones shined through?</p>
      ...
      <div className="bg-red-500 size-4/5 flex flex-col items-center m-6 rounded-md h-72 justify-evenly pb-2">
        <p className="text-background">Creator's question</p>
        <p className="text-background w-11/12">{artistQuestion}</p>
